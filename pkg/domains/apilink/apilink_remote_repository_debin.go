...
func (r *remoteRepository) GetDebinesApiLink(requerimientoId string, request linkdebin.RequestGetDebinesLink, token string) (response linkdebin.ResponseGetDebinesLink, erro error) {
	...
	*/
	...
	if err != nil {
		logs.Error("Error al ejecutar apilink Get debines"+ err.Error())
		logError := entities.Log{
			Tipo:          entities.EnumLog("error"),
			Funcionalidad: "GetConsultarDebines",
			Mensaje:       fmt.Sprintf( "%s, el token que se consulto es: %s ",err,token),
		}
		errCrearLog := r.UtilService.CreateLogService(logError)
		if errCrearLog != nil {
