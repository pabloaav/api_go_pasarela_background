...
func (s *aplinkService) CreateTransferenciaApiLinkService(requerimientoId, token string, request linktransferencia.RequestTransferenciaCreateLink) (response linktransferencia.ResponseTransferenciaCreateLink, erro error) {
	...
	// NOTE el token se debe recibir por parametros
	//response, erro = s.remoteRepository.CreateTransferenciaApiLink(requerimientoId, request, token.AccessToken)
	response, erro = s.remoteRepository.CreateTransferenciaApiLink(requerimientoId, request, token)
