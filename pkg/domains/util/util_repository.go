...
type UtilRepository interface {
	...
	GetSubcuentasByIdCuenta(id uint) (subcuentas []entities.Subcuenta, erro error)
	GetImpuestoByIdRepository(id int64) (impuesto entities.Impuesto, erro error)

	GetCorreosTelcoRepository() (correos []entities.TelcoCorreo, erro error)
}
...
func (r *utilRepository) GetSubcuentasByIdCuenta(id uint) (subcuentas []entities.Subcuenta, erro error) {
	resp := r.SqlClient.Where("cuentas_id = ?", id).Find(&subcuentas)
	resp.Preload("Cuenta")

	if resp.Error != nil {
		erro = fmt.Errorf("error al obtener subcuenta %s", resp.Error.Error())
	}
	return
...
func (r *utilRepository) GetCorreosTelcoRepository() (correos []entities.TelcoCorreo, erro error) {
	resp := r.SqlClient.Model(entities.TelcoCorreo{}).Find(&correos)
	if resp.Error != nil {
		erro = fmt.Errorf("error al obtener correos Telco: %s", resp.Error.Error())
		return
	}
	return
}
