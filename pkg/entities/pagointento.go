...
type Pagointento struct {
	...
	NotificadoOnline     bool                  `json:"notificado_online"`
...
// retorna true si el pagointento tiene fecha de reversion igual o posterior a la fecha de inicio de actividades de Wee como agente de retencion
func (p *Pagointento) RevertirMovimientoRetencion() (res bool) {
	// fecha de inicio de actividades de Wee como agente de retencion
	fechaInicio := time.Date(2023, time.November, 1, 0, 0, 0, 0, time.UTC)
	if p.RevertedAt.After(fechaInicio) || p.RevertedAt == fechaInicio {
		res = true
	}
	return
}