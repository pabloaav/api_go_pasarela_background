...
type ResponsePagosItems struct {
	...
	Fecha          string
	Pagos          []entities.Pago
	PagosCobranzas []DetallesPagosCobranza
	PagLotes       PagLotes
}
...
// Usado para archivo batch pagos general

type ResultPagosItemsAlternativo struct {
	PagLotes        PagLotes
	CabeceraArchivo CabeceraArchivo
	CabeceraLote    CabeceraLote
	ResultItems     []ResultItemsAlternativo
	ColaArchivo     ColaArchivo
	Clienteid       uint
	EmailsBatch     []string
}

type ResultItemsAlternativo struct {
	DetalleTransaccion DetalleTransaccion
	DetalleDescripcion DetalleDescripcionAlternativo
}

type DetalleDescripcionAlternativo struct {
	RecordCodeLote string `json:"record_code_lote"` // 1   codigo de registro inicia con 6
	UUID           string `json:"uuid"`             // 80   Codigo único de transaccion
	TypeCode       string `json:"type_code"`        // 1 este campo no es obligatorio
	Filler         string `json:"filler"`           // 46 este campo no es obligatorio
}

type ResponsePagosItemsAlternativo struct {
	Clientes       ClientesResponse
	Fecha          string
	Pagos          []entities.Pago
	PagosCobranzas []DetallesPagosCobranza
	PagLotes       PagLotes
	BatchContactos []string
}
...
func (cabDesc *DetalleDescripcionAlternativo) ValidarDetalleDescripcionAlternativo(estructuraReg *EstructuraRegistrosBatch) (erro error) {
	err := errors.New("longitud del campo es incorrecto para el registro detalle descripción")
	if len(cabDesc.RecordCodeLote) != estructuraReg.DetalleDescripcionAlternativo()[0].Cantidad {
		return err
	}
	if len(cabDesc.UUID) != estructuraReg.DetalleDescripcionAlternativo()[1].Cantidad {
		return err
	}
	if len(cabDesc.TypeCode) != estructuraReg.DetalleDescripcionAlternativo()[2].Cantidad {
		return err
	}
	if len(cabDesc.Filler) != estructuraReg.DetalleDescripcionAlternativo()[3].Cantidad {
		return err
	}
	return
}
